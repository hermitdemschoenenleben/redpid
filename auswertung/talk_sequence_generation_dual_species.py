...
def iterate_over_control_signals(folder):
    filename = folder + 'control-signal-dual-species.pickle'
...
def iterate_over_frequencies(folder):
    ...
    for i, n in enumerate(list(int(_) for _ in np.arange(0, 16001, 300))):
    #for i, n in enumerate(list(int(_) for _ in [100, 2000, 4000, 6000, 8000, 10000, 12000, 14000, 16000])):
        print(i, n)
...
def plot_control_signal_and_frequencies(folder):
    ...
    for i, [[times_c, control], [times_f, frequencies]] in enumerate(zip(
        ...
        len_f = int(len(times_f) / 500 * time_length)
        shift = 100

        plt.plot((-1000, 10000), (6.835, 6.835), 'k--')
        plt.plot((-1000, 10000), (.25, .25), 'k--')
        plt.plot((-1000, 10000), (1.370, 1.370), 'k--')
        plt.plot((-1000, 10000), (4.39, 4.41), 'k--')
...
if __name__ == '__main__':
    folder = '/media/depot/data-2019/afmot/jump-generation-dual-species/'
