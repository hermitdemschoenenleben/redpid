...
def load_density_data(folder):
    ...
    return relations, mot_densities, afmot_densities
...
folder = '/media/depot/data/afmot/density/'
folder = '/home/ben/Schreibtisch/letzter-test/'

data = []
data_std = []
#for fn in ('24.4MHz', '24.6MHz', '24.8MHz'):
for fn in ('',):
    relations, mot_densities, afmot_densities = load_density_data(folder + fn + '/')
    data.append(np.mean(relations))
    data_std.append(np.std(relations))
...
plt.show()
plt.plot([24.4, 24.6, 24.8], data)
plt.errorbar([24.4, 24.6, 24.8], data, yerr=data_std)
plt.xlabel('detuning in MHz/10')
